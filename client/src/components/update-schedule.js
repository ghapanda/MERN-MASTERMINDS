...
function SessionContainer({ index, sessionData, onSessionChange }) {
  ...
  return (
    <div className="session-container">
      <label htmlFor="name">Name: </label> <input id="name" value={data.name} onChange={handleChange} />
      <label htmlFor="week">Week: </label> <input id="week" value={data.week} onChange={handleChange} />
      <label htmlFor="day">Day: </label> <input id="day" value={data.day} onChange={handleChange} />
      <label htmlFor="date">Date: </label> <input id="date" value={data.date} onChange={handleChange} />
      <label htmlFor="time">Time: </label> <input id="time" value={data.time} onChange={handleChange} />
      <label htmlFor="location">Location: </label> <input id="location" value={data.location} onChange={handleChange} />
      <label htmlFor="contact">Contact: </label> <input id="contact" value={data.contact} onChange={handleChange} />
    </div>
  );
}
...
function UpdateSchedule() {
  ...
  useEffect(() => {
    ...
    axios.get('http://localhost:3001/api/update-schedule')
      .then(response => {
  ...
  const update = () => {
    axios.post('http://localhost:3001/api/update-schedule', sessions)
      .then(response => {
